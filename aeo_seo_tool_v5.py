#!/usr/bin/env python3
"""
AEO/SEO Audit & Auto-Fix Tool v5.0
=====================================
...
Käyttö:
    python aeo_seo_tool_v5.py --repo /polku/sivustoon
    python aeo_seo_tool_v5.py --repo . --fix
    python aeo_seo_tool_v5.py --url https://example.com --max-pages 10
    python aeo_seo_tool_v5.py --url https://example.com --klar-json
    python aeo_seo_tool_v5.py --repo . --fix --cuisine "italialainen" --phone "+358 40 123 4567"
"""
...
<footer>
  ...
  {esc(COSTS.summary())}
    ...
    out_file = output_path / f"audit_v5_{safe}_{ts}.html"
...
def save_reports(
    ...
    # JSON
    json_path = out / f"audit_v5_{safe}_{ts_file}.json"
    ...
    # Markdown
    md_path = out / f"audit_v5_{safe}_{ts_file}.md"
    recs = recommendations(all_checks)
...
def audit_url(url: str, fix: bool, out: Path, site_args: SiteContext,
              max_pages: int = 10, klar_json: bool = False):
    ...
    for fn, page_html in pages.items():
        ...
        if fix:
            ...
            if fixer.applied:
                ...
                fixed_path = out / f"fixed_v5_{ts_file}_{fn}"
