#!/usr/bin/env python3
"""
AEO/SEO Audit & Auto-Fix Tool v7.0
=====================================
...
V7: Ennen/jälkeen-vertailuraportti (--compare vanha_audit.json) —
    kuukausiraportointi asiakkaalle: pistekehitys, parannukset ja
    jäljellä olevat puutteet (VERTAILU.html + md + json)

Käyttö:
    python aeo_seo_tool_v7.py --repo /polku/sivustoon
    python aeo_seo_tool_v7.py --repo . --fix
    python aeo_seo_tool_v7.py --url https://example.com --max-pages 10
    python aeo_seo_tool_v7.py --url https://example.com --fix-guide --klar-json
    python aeo_seo_tool_v7.py --url https://example.com --compare reports/audit_v6_..._.json
"""
...
{{
  ...
  "price_range": "€, €€ tai €€€ jos hintataso käy ilmi, muuten tyhjä"
    ...
    def progress_summary(self, old_pct: float, new_pct: float,
                         improved: List[str], remaining: List[str]) -> str:
        """V7: 2–3 lauseen selkokielinen kehityskuvaus kuukausiraporttiin."""
        prompt = f"""Olet hakukonenäkyvyyden konsultti. Kirjoita asiakkaalle 2–3 lauseen
selkokielinen yhteenveto sivuston {self.site.name or self.site.url or ''} kehityksestä
seurantajaksolla. Sävy myönteinen ja rakentava; ei teknisiä termejä; jäljellä
olevat puutteet esitetään "korjataan seuraavaksi" -hengessä, ei moitteena.

Pisteet: {old_pct:.0f}/100 → {new_pct:.0f}/100
Parannukset jaksolla: {', '.join(improved[:8]) if improved else 'ei muutoksia'}
Vielä työn alla: {', '.join(remaining[:5]) if remaining else 'ei avoimia puutteita'}

Vastaa suomeksi, korkeintaan 60 sanaa. Palauta VAIN teksti."""
        return self._ask(prompt, 200)
...
<footer>
  ...
  {esc(COSTS.summary())}
    ...
    out_file = output_path / f"audit_v7_{safe}_{ts}.html"
...
def results_to_pages_dict(results: Dict[str, List[Check]]) -> dict:
    """V7: tulokset audit-JSONin pages-rakenteeksi. Jaettu save_reportsin ja
    --compare-vertailun kesken, jotta vertailu toimii täsmälleen samalla
    muodolla jonka työkalu tallentaa."""
    return {
        fname: {
            "score": score_pct(chs),
...
def save_reports(
    ...
    # JSON
    json_path = out / f"audit_v7_{safe}_{ts_file}.json"
    ...
    # Markdown
    md_path = out / f"audit_v7_{safe}_{ts_file}.md"
    recs = recommendations(all_checks)
...
# ── V7: Ennen/jälkeen-vertailu (--compare) ──────────────────────────────────

STATUS_ORDER = {"fail": 0, "warn": 1, "pass": 2}


def load_old_audit(path: Path) -> dict:
    """Lukee ja validoi aiemman audit-JSONin. Selkeä virhe ja exit, jos
    tiedosto ei kelpaa — ajetaan ennen auditointia (fail fast)."""
    try:
        data = json.loads(Path(path).read_text(encoding="utf-8"))
    except OSError as e:
        _err(f"Vertailutiedostoa ei voitu lukea: {e}")
        sys.exit(1)
    except json.JSONDecodeError as e:
        _err(f"Vertailutiedosto ei ole validia JSONia: {path} ({e})")
        sys.exit(1)
    if not isinstance(data, dict) or "pages" not in data or "overall_score" not in data:
        _err(f"Tiedosto ei näytä työkalun audit-JSONilta (pages/overall_score puuttuu): {path}")
        sys.exit(1)
    return data


def compare_audits(old: dict, new_pages: dict, new_overall: float,
                   new_timestamp: str = "") -> dict:
    """Vertaa aiempaa audit-JSONia uuden ajon tuloksiin. Puhdas funktio.

    Muutosluokat (statussiirtymät, fail=0 < warn=1 < pass=2):
      improved   = fail/warn → pass tai fail → warn
      regressed  = warn → fail (puute paheni)
      new_issues = pass → warn/fail (uusi puute ilmestyi)
      new_checks = tarkistus vain uudessa ajossa (esim. uudempi työkaluversio)
                   — EI koskaan improved/regressed/new_issues-listoihin
    Vanhasta puuttuvat sivut/tarkistukset raportoidaan erikseen, ei kaadeta."""
    old_pages = old.get("pages", {})

    def entry(name: str, old_c: Optional[dict], new_c: dict) -> dict:
        return {"name": name, "plain": plain_name(name),
                "from": old_c["status"] if old_c else None,
                "to": new_c["status"], "message": new_c.get("message", "")}

    pages: Dict[str, dict] = {}
    removed_checks: List[str] = []
    # Vertailukelpoiset pisteet: vain molemmissa ajoissa olevat tarkistukset
    c_old_s = c_old_m = c_new_s = c_new_m = 0

    for fname, ndata in new_pages.items():
        odata = old_pages.get(fname)
        if odata is None:
            continue  # uusi sivu — raportoidaan new_pages-listassa
        oldc = {c["name"]: c for c in odata.get("checks", [])}
        newc = {c["name"]: c for c in ndata.get("checks", [])}
        improved, regressed, new_issues, new_checks = [], [], [], []
        for name, nc in newc.items():
            oc = oldc.get(name)
            if oc is None:
                new_checks.append(entry(name, None, nc))
                continue
            c_old_s += oc.get("score", 0); c_old_m += oc.get("max", 0)
            c_new_s += nc.get("score", 0); c_new_m += nc.get("max", 0)
            o = STATUS_ORDER.get(oc.get("status"), 1)
            n = STATUS_ORDER.get(nc.get("status"), 1)
            if n > o:
                improved.append(entry(name, oc, nc))
            elif o == 1 and n == 0:
                regressed.append(entry(name, oc, nc))
            elif o == 2 and n < 2:
                new_issues.append(entry(name, oc, nc))
        removed_checks += [f"{fname}: {n}" for n in oldc if n not in newc]
        pages[fname] = {
            "before": odata.get("score", 0), "now": ndata.get("score", 0),
            "delta": round(ndata.get("score", 0) - odata.get("score", 0), 1),
            "improved": improved, "regressed": regressed,
            "new_issues": new_issues, "new_checks": new_checks,
        }

    # Jäljellä olevat puutteet uudesta ajosta (fail ensin, sitten warn)
    remaining = []
    for fname, ndata in new_pages.items():
        for c in ndata.get("checks", []):
            if c.get("status") != "pass":
                remaining.append({"page": fname, "name": c["name"],
                                  "plain": plain_name(c["name"]),
                                  "status": c["status"],
                                  "message": c.get("message", "")})
    remaining.sort(key=lambda r: (STATUS_ORDER.get(r["status"], 1), r["page"]))

    comparable_delta = None
    if c_old_m and c_new_m:
        comparable_delta = round(c_new_s / c_new_m * 100 - c_old_s / c_old_m * 100, 1)

    old_overall = old.get("overall_score", 0) or 0
    return {
        "old": {"timestamp": old.get("timestamp", ""),
                "tool_version": old.get("tool_version", "?"),
                "overall_score": old_overall},
        "new": {"timestamp": new_timestamp, "tool_version": VERSION,
                "overall_score": round(new_overall, 1)},
        "score_delta": round(new_overall - old_overall, 1),
        "comparable_delta": comparable_delta,
        "pages": pages,
        "new_pages": sorted(p for p in new_pages if p not in old_pages),
        "removed_pages": sorted(p for p in old_pages if p not in new_pages),
        "removed_checks": removed_checks,
        "remaining_issues": remaining,
    }


def _cmp_score_color(pct: float) -> str:
    if pct >= 85: return "#16a34a"
    if pct >= 70: return "#d97706"
    return "#dc2626"


def write_comparison_outputs(cmp: dict, site_label: str, out: Path,
                             ai_summary: str = "") -> Path:
    """Kirjoittaa vertailukansion: VERTAILU.html (asiakkaalle, Anglés-tyyli),
    VERTAILU.md (sähköpostiin) ja vertailu.json. Palauttaa kansion polun.
    Sävy on rakentava: puutteet esitetään "korjataan seuraavaksi" -hengessä."""
    esc = html_mod.escape
    ts_file = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    safe = re.sub(r"[^\w]", "_", site_label)[:35]
    vdir = out / f"vertailu_{safe}_{ts_file}"
    vdir.mkdir(parents=True, exist_ok=True)

    old_pct = cmp["old"]["overall_score"]
    new_pct = cmp["new"]["overall_score"]
    delta = cmp["score_delta"]
    old_date = cmp["old"]["timestamp"] or "aiempi ajo"
    new_date = cmp["new"]["timestamp"] or datetime.datetime.now().strftime("%d.%m.%Y %H:%M")

    all_improved = [(fn, e) for fn, p in sorted(cmp["pages"].items()) for e in p["improved"]]
    all_attention = [(fn, e) for fn, p in sorted(cmp["pages"].items())
                     for e in (p["new_issues"] + p["regressed"])]
    all_new_checks = [(fn, e) for fn, p in sorted(cmp["pages"].items()) for e in p["new_checks"]]
    n_new_checks = len(all_new_checks)

    # Priorisoitu "korjataan seuraavaksi" recommendations()-järjestyksellä
    shim = [Check(name=r["name"], category="", score=0, max_score=0,
                  status=r["status"], message="") for r in cmp["remaining_issues"]]
    rec_texts = recommendations(shim)

    # ── VERTAILU.html ──
    delta_badge = (f'+{delta:g} pistettä' if delta > 0 else
                   (f'{delta:g} pistettä' if delta < 0 else "pisteet ennallaan"))
    note_html = ""
    if n_new_checks:
        note_html = (f'<p class="meta" style="margin-top:10px">Huomio: seurantaan on '
                     f'lisätty {n_new_checks} uutta tarkistusta edellisen ajon jälkeen — '
                     f'ne eivät ole mukana vertailussa.'
                     + (f' Vertailukelpoinen muutos yhteisillä tarkistuksilla: '
                        f'{cmp["comparable_delta"]:+g} pistettä.' if cmp["comparable_delta"] is not None else "")
                     + '</p>')

    improved_html = ""
    if all_improved:
        items = "".join(
            f'<li>✅ <strong>{esc(e["plain"])}</strong> · {esc(fn)}</li>'
            for fn, e in all_improved)
        improved_html = f'<section class="page"><h2>Mitä parani tässä jaksossa</h2><ul class="clean">{items}</ul></section>'
    else:
        improved_html = ('<section class="page"><h2>Mitä parani tässä jaksossa</h2>'
                         '<p>Ei muutoksia tarkistusten tiloissa tällä jaksolla — '
                         'tilanne on pysynyt vakaana.</p></section>')

    attention_html = ""
    if all_attention:
        items = "".join(
            f'<li>🔧 <strong>{esc(e["plain"])}</strong> · {esc(fn)} — nostettu työlistan kärkeen</li>'
            for fn, e in all_attention)
        attention_html = ('<section class="page"><h2>Uutta työlistalla</h2>'
                          '<p>Nämä kohdat ovat tulleet esiin edellisen ajon jälkeen — '
                          'ne korjataan seuraavaksi:</p>'
                          f'<ul class="clean">{items}</ul></section>')

    new_checks_html = ""
    if all_new_checks:
        seen_names = sorted({e["plain"] for _, e in all_new_checks})
        items = "".join(f'<li>🆕 {esc(n)}</li>' for n in seen_names)
        new_checks_html = ('<section class="page"><h2>Uudet tarkistukset seurannassa</h2>'
                           '<p>Seurantaa on laajennettu — nämä tarkistukset ovat mukana '
                           'ensimmäistä kertaa eivätkä vaikuta yllä olevaan vertailuun:</p>'
                           f'<ul class="clean">{items}</ul></section>')

    # Tarkistukset, joita recommendations()-prioriteettilista ei kata
    # (esim. julkaisukunto, varauswidget) — listataan selkonimellä perään
    def _rec_covers(name: str) -> bool:
        return bool(recommendations([Check(name=name, category="", score=0,
                                           max_score=0, status="warn", message="")]))
    extra = sorted({r["plain"] for r in cmp["remaining_issues"]
                    if not _rec_covers(r["name"])})
    next_rows = "".join(f'<li>{esc(t)}</li>' for t in rec_texts)
    next_rows += "".join(f'<li>{esc(n)}</li>' for n in extra)
    next_html = ""
    if next_rows:
        next_html = ('<section class="page"><h2>Korjataan seuraavaksi</h2>'
                     '<p>Jäljellä olevat kehityskohteet tärkeysjärjestyksessä:</p>'
                     f'<ol>{next_rows}</ol></section>')

    pages_note = ""
    if cmp["new_pages"] or cmp["removed_pages"]:
        bits = []
        if cmp["new_pages"]:
            bits.append("uusia sivuja seurannassa: " + ", ".join(cmp["new_pages"]))
        if cmp["removed_pages"]:
            bits.append("edellisessä ajossa mukana olleita sivuja ei enää löytynyt: "
                        + ", ".join(cmp["removed_pages"]))
        pages_note = f'<p class="meta" style="margin-top:8px">({esc("; ".join(bits))})</p>'

    score_rows = "".join(
        f'<tr><td>{esc(fn)}</td><td style="text-align:right">{p["before"]:.0f}</td>'
        f'<td style="text-align:right">{p["now"]:.0f}</td>'
        f'<td style="text-align:right;font-weight:700;color:'
        f'{"#16a34a" if p["delta"] > 0 else ("#dc2626" if p["delta"] < 0 else "#555")}">'
        f'{p["delta"]:+.0f}</td></tr>'
        for fn, p in sorted(cmp["pages"].items()))

    ai_html = ""
    if ai_summary:
        paras = "".join(f"<p>{esc(x)}</p>" for x in ai_summary.split("\n") if x.strip())
        ai_html = f'<div class="intro">{paras}</div>'

    html = f"""<!DOCTYPE html>
<html lang="fi"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Kehitysraportti — {esc(site_label)}</title>
<style>{ANGLES_CSS}
.scores{{display:flex;align-items:center;gap:16px;margin:6px 0 10px}}
.sbox{{flex:1;background:#f4fafd;border:1px solid #d5e9f5;border-radius:14px;padding:22px 16px;text-align:center}}
.sbox.now{{border:2px solid #89CFF0}}
.sbox .lbl{{font-size:.72rem;text-transform:uppercase;letter-spacing:.08em;color:#555}}
.sbox .num{{font-size:2.9rem;font-weight:800;line-height:1.15}}
.sbox .date{{color:#555;font-size:.82rem;margin-top:6px}}
.cmp-arrow{{font-size:1.7rem;color:#89CFF0;flex-shrink:0}}
.delta-badge{{display:inline-block;background:#89CFF0;color:#111;font-weight:700;
  font-size:.85rem;padding:5px 16px;border-radius:99px;margin-top:6px}}
ul.clean{{list-style:none;padding-left:0}}
table.scoret{{width:100%;border-collapse:collapse;margin-top:8px}}
table.scoret th{{text-align:left;font-size:.72rem;text-transform:uppercase;color:#555;
  border-bottom:2px solid #89CFF0;padding:6px 8px}}
table.scoret th:not(:first-child){{text-align:right}}
table.scoret td{{padding:7px 8px;border-bottom:1px solid #eee;font-size:.9rem}}
</style></head><body>

<header>
  <h1>Kehitysraportti — {esc(site_label)}</h1>
  <div class="meta">Vertailujakso: {esc(old_date)} → {esc(new_date)}</div>
</header>

<div class="scores">
  <div class="sbox">
    <div class="lbl">Ennen</div>
    <div class="num" style="color:{_cmp_score_color(old_pct)}">{old_pct:.0f}</div>
    <div class="date">{esc(old_date)}</div>
  </div>
  <div class="cmp-arrow">→</div>
  <div class="sbox now">
    <div class="lbl">Nyt</div>
    <div class="num" style="color:{_cmp_score_color(new_pct)}">{new_pct:.0f}</div>
    <div class="date">{esc(new_date)}</div>
    <div><span class="delta-badge">{esc(delta_badge)}</span></div>
  </div>
</div>
{note_html}
{ai_html}
...
<section class="page">
  <h2>Sivukohtaiset pisteet</h2>
  <table class="scoret">
    <thead><tr><th>Sivu</th><th>Ennen</th><th>Nyt</th><th>Muutos</th></tr></thead>
...
<footer>
  Kehitysraportin laati <strong>Anglés Marketing</strong> · {esc(new_date)}
</footer>
</body></html>"""
    (vdir / "VERTAILU.html").write_text(html, encoding="utf-8")

    # ── VERTAILU.md ──
    md = [f"# Kehitysraportti — {site_label}", "",
          f"**Vertailujakso:** {old_date} → {new_date}  ",
          f"**Pisteet:** {old_pct:.0f}/100 → {new_pct:.0f}/100 ({delta:+g})  "]
    if n_new_checks:
        md.append(f"*Seurantaan lisätty {n_new_checks} uutta tarkistusta — eivät mukana vertailussa."
                  + (f" Vertailukelpoinen muutos: {cmp['comparable_delta']:+g} p.*"
                     if cmp["comparable_delta"] is not None else "*"))
    if ai_summary:
        md += ["", ai_summary]
    md += ["", "## Mitä parani tässä jaksossa", ""]
    if all_improved:
        md += [f"- ✅ {e['plain']} · {fn}" for fn, e in all_improved]
    else:
        md += ["- Ei muutoksia tarkistusten tiloissa — tilanne pysynyt vakaana"]
    if all_attention:
        md += ["", "## Uutta työlistalla (korjataan seuraavaksi)", ""]
        md += [f"- 🔧 {e['plain']} · {fn}" for fn, e in all_attention]
    if all_new_checks:
        md += ["", "## Uudet tarkistukset seurannassa", ""]
        md += [f"- 🆕 {n}" for n in sorted({e['plain'] for _, e in all_new_checks})]
    if rec_texts or extra:
        md += ["", "## Korjataan seuraavaksi", ""]
        md += [f"{i}. {t}" for i, t in enumerate(rec_texts + extra, 1)]
    md += ["", "## Sivukohtaiset pisteet", "",
           "| Sivu | Ennen | Nyt | Muutos |", "|---|---|---|---|"]
    md += [f"| {fn} | {p['before']:.0f} | {p['now']:.0f} | {p['delta']:+.0f} |"
           for fn, p in sorted(cmp["pages"].items())]
    if cmp["new_pages"]:
        md += ["", f"*Uusia sivuja seurannassa: {', '.join(cmp['new_pages'])}*"]
    if cmp["removed_pages"]:
        md += [f"*Sivuja ei enää löytynyt: {', '.join(cmp['removed_pages'])}*"]
    md += ["", "---", "Kehitysraportin laati **Anglés Marketing**"]
    (vdir / "VERTAILU.md").write_text("\n".join(md) + "\n", encoding="utf-8")

    # ── vertailu.json ──
    out_json = dict(cmp)
    out_json["ai_summary"] = ai_summary or None
    (vdir / "vertailu.json").write_text(
...
# V7: OHJEET.html:n ja VERTAILU.html:n yhteinen vaalea Anglés-tyyli
ANGLES_CSS = """
*{box-sizing:border-box;margin:0;padding:0}
...
<style>{ANGLES_CSS}</style></head><body>
...
# ── V7: vertailuajo (jaettu repo- ja URL-ajurin kesken) ─────────────────────

def run_comparison(compare_old: dict, results: Dict[str, List[Check]],
                   site: SiteContext, out: Path, label: str):
    """Vertaa ajon lopulliset tulokset aiempaan audit-JSONiin ja kirjoittaa
    vertailukansion (VERTAILU.html + md + json)."""
    old_ver = compare_old.get("tool_version", "?")
    if old_ver != VERSION:
        _info(f"Vertailu eri työkaluversioon (v{old_ver} → v{VERSION}) — "
              "uudet tarkistukset raportoidaan erikseen, eivät vertailussa")
    new_pages = results_to_pages_dict(results)
    all_checks = [c for chs in results.values() for c in chs]
    s, m = scores(all_checks)
    new_overall = round(s / m * 100, 1) if m else 0.0
    cmp = compare_audits(compare_old, new_pages, new_overall,
                         datetime.datetime.now().strftime("%d.%m.%Y %H:%M"))

    ai_txt = ""
    if AI_AVAILABLE:
        _info("AI kirjoittaa kehitysyhteenvetoa...")
        improved = [e["plain"] for p in cmp["pages"].values() for e in p["improved"]]
        remaining = [r["plain"] for r in cmp["remaining_issues"]]
        ai_txt = AIWriter(site).progress_summary(
            cmp["old"]["overall_score"], new_overall, improved, remaining)

    vdir = write_comparison_outputs(cmp, site.name or site.url or label, out, ai_txt)
    n_impr = sum(len(p["improved"]) for p in cmp["pages"].values())
    _ok(f"Vertailu: {cmp['old']['overall_score']:.0f} → {new_overall:.0f} pistettä "
        f"({cmp['score_delta']:+g}) · {n_impr} parannusta")
    _ok(f"Vertailuraportti: {vdir}/")


# ── Repo-ajuri ───────────────────────────────────────────────────────────────

def audit_repo(repo: Path, fix: bool, out: Path, site_args: SiteContext, open_report: bool = True,
               klar_json: bool = False, compare_old: Optional[dict] = None):
    ...
    if compare_old is not None:
        run_comparison(compare_old, results, site, out, str(repo))
...
def audit_url(url: str, fix: bool, out: Path, site_args: SiteContext,
        ...
        if fix:
            ...
            if fixer.applied:
                ...
                if pkg_dir is not None:
                    ...
                    fixed_path = out / f"fixed_v7_{ts_file}_{fn}"
    ...
    if compare_old is not None:
        run_comparison(compare_old, results, site, out, url)
...
""")
    ...
    # V7: ennen/jälkeen-vertailu
    p.add_argument("--compare",    default="", metavar="POLKU",
                   help="Vertaa ajon tulosta aiempaan audit-JSONiin (audit_*.json) ja "
                        "tuota kehitysraportti asiakkaalle (VERTAILU.html + md + json)")
    ...
    # V7: lue ja validoi vertailu-JSON ennen auditointia (fail fast)
    compare_old = load_old_audit(Path(args.compare)) if args.compare else None

    if args.url:
        audit_url(args.url, args.fix or args.fix_guide, out, site_args,
                  max_pages=max(1, args.max_pages), klar_json=args.klar_json,
                  fix_guide=args.fix_guide, compare_old=compare_old)
    else:
        if args.fix_guide:
        ...
        audit_repo(repo, args.fix, out, site_args, open_report=not args.no_open,
                   klar_json=args.klar_json, compare_old=compare_old)
