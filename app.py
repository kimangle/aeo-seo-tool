"""V8: audit-api — WSGI-sovellus, joka tarjoaa SEO-testin ja lomakkeen reitit.
  ...
  GET  /dev          — kehittäjäkonsoli: asiakasnäkymä (dev_console.py)
...
from web_common import (allowed_origins, cors_origin, json_response, read_json,
                        validate_target)
...
def _audit(environ, start_response, origin):
    ...
    if body.get("website"):
        return json_response(start_response, 202, {"ok": True}, origin)
    ...
    try:
        ...
        return json_response(start_response, 422, {"error": str(e)}, origin)
    except Exception:
...
def _lead(environ, start_response, origin):
    ...
    if not key or not group:
        return json_response(start_response, 503,
    ...
    try:
        ...
        ok = False
    if not ok:
        return json_response(start_response, 502,
                              {"error": "Lähetys epäonnistui — yritä uudelleen"}, origin)
    return json_response(start_response, 200, {"ok": True}, origin)
...
def _contact(environ, start_response, origin):
    ...
    if not 1 <= len(nimi) <= 120:
        return json_response(start_response, 400, {"error": "Kerro nimesi"}, origin)
    if not EMAIL_RE.match(email):
        return json_response(start_response, 400,
                              {"error": "Tarkista sähköpostiosoite"}, origin)
    if len(viesti) > 4000:
...
def _health(start_response):
    ...
    return json_response(start_response, 200, {
        ...
        "dev_console_enabled": bool(os.environ.get("DEV_TOKEN", "")),
...
def app(environ, start_response):
    ...
    if method == "GET" and path == "/dev":
        return dev_console.page(start_response)
    if method == "GET" and path == "/api/dev/clients":
        return dev_console.clients(environ, start_response)
    if method == "POST" and path == "/api/dev/audit":
        return dev_console.audit(environ, start_response)
    if method == "OPTIONS":
