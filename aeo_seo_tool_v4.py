#!/usr/bin/env python3
"""
AEO/SEO Audit & Auto-Fix Tool v4.0
=====================================
...
V4: Ravintola-schema (Restaurant JSON-LD) · Sivustotason tiedostot
    (robots.txt, sitemap.xml, llms.txt) tarkistuksineen ja generointeineen

Käyttö:
    python aeo_seo_tool_v4.py --repo /polku/sivustoon
    python aeo_seo_tool_v4.py --repo . --fix
    python aeo_seo_tool_v4.py --url https://example.com
    python aeo_seo_tool_v4.py --repo . --fix --site-url https://example.com --site-name "Yritys"
    python aeo_seo_tool_v4.py --repo . --fix --cuisine "italialainen" --phone "+358 40 123 4567"
"""
...
<footer>
  ...
  {esc(COSTS.summary())}
    ...
    out_file = output_path / f"audit_v4_{safe}_{ts}.html"
...
def save_reports(
    ...
    # JSON
    json_path = out / f"audit_v4_{safe}_{ts_file}.json"
    ...
    # Markdown
    md_path = out / f"audit_v4_{safe}_{ts_file}.md"
    recs = recommendations(all_checks)
...
def audit_repo(repo: Path, fix: bool, out: Path, site_args: SiteContext, open_report: bool = True):
    ...
    if fix:
        ...
        if applied:
            ...
            site_checks = site_file_checks_repo(repo)  # uudelleentarkistus
...
def audit_url(url: str, fix: bool, out: Path, site_args: SiteContext):
    ...
    site = detect_site_context([url_soup], site_args.url or url, site_args.name, site_args.email)
    site = merge_cli_site_args(site, site_args)
    if not site.url:
        parsed = urlparse(url)
    ...
    if site.is_restaurant():
        _info("Sivusto tunnistettu ravintolaksi — Restaurant-schema-tarkistus käytössä")
        if fix:
            enrich_restaurant_details(site, url_soup.get_text(" ", strip=True))
    ...
    if fix:
        ...
        if fixer.applied:
            fixes_applied[fname] = fixer.applied
            ts_file = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            fixed_path = out / f"fixed_v4_{ts_file}.html"
    ...
    site_checks = site_file_checks_url(base)
    results[SITE_PSEUDO_PAGE] = site_checks
    print_results(site_checks, "Sivustotason tiedostot")
    if fix and any(c.auto_fixable and c.status != "pass" for c in site_checks):
        ...
        dest = out / "sivustotiedostot"
        applied = generate_site_files(dest, site, {fname: html}, site_checks)
        if applied:
...
def main():
    p = argparse.ArgumentParser(
        ...
        epilog="""Esimerkkejä:
  python aeo_seo_tool_v4.py --repo ./sivusto
  python aeo_seo_tool_v4.py --repo . --fix
  python aeo_seo_tool_v4.py --url https://example.com
  python aeo_seo_tool_v4.py --repo . --fix --site-url https://oma.fi --site-name "Yritys Oy"
  python aeo_seo_tool_v4.py --repo . --fix --restaurant --cuisine "italialainen" \\
